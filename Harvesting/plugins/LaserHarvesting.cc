	...
	DQHarvester(ps)
{
	...
	for (uint32_t i=0; i<_vruns.size(); i++)
	{
		...
		_vcTimingMean_depth[i].initialize("LaserTask",
			"TimingMean", hashfunctions::fdepth,
			new quantity::DetectorQuantity(quantity::fieta),
			new quantity::DetectorQuantity(quantity::fiphi),
			new quantity::ValueQuantity(quantity::fTiming_TS200));
		_vcSignalMean_depth[i].initialize("LaserTask",
		...
		//	 init new containers
		_vcSignalRatio_SubdetPM[i].initialize("LaserTrend", "SignalRatio",
			hashfunctions::fSubdetPM,
			new quantity::ValueQuantity(quantity::fRatio2),
			new quantity::ValueQuantity(quantity::fN, true));
		_vcSignalRatio_depth[i].initialize("LaserTrend", "SignalRatio",
...
/*	if (_htype==fClientLike)
	{
		_vcTimingMean_depth[_vruns.size()].initialize("LaserTask",
			"TimingMean", hashfunctions::fdepth,
			new quantity::DetectorQuantity(quantity::fieta),
			new quantity::DetectorQuantity(quantity::fiphi),
			new quantity::ValueQuantity(quantity::fTiming_TS200));
		_vcSignalMean_depth[_vruns.size()].initialize("LaserTask",
			"SignalMean", hashfunctions::fdepth,
			new quantity::DetectorQuantity(quantity::fieta),
			new quantity::DetectorQuantity(quantity::fiphi),
			new quantity::ValueQuantity(quantity::ffC_3000));
...
void LaserHarvesting::book(DQMStore* store)
{
...
/* virtual */ void LaserHarvesting::endRun(edm::Run const& r, 
...
DEFINE_FWK_MODULE(LaserHarvesting);
