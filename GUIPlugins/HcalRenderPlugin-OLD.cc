...
private:
  ...
  void postDrawTH2( TCanvas *c, const VisDQMObject &o )
  {
    ...
    if (o.name.find("channel status/Channels Summary" ) != std::string::npos){
        c->SetTopMargin(0);
        c->SetRightMargin(0);
	c->SetGridy();
	if(o.name.find("Low energy"  ) != std::string::npos || 
	   o.name.find("Bad timing"  ) != std::string::npos || 
	   o.name.find("Bad pedestal mean"  ) != std::string::npos || 
	   o.name.find("Bad pedestal rms"  ) != std::string::npos || 
	   o.name.find("missing VME" ) != std::string::npos || 
	   o.name.find("VME-uTCA mismatches" )  != std::string::npos 
	) c->SetLeftMargin(0);
	else if(o.name.find("Missing") != std::string::npos || 
		o.name.find("missing uTCA" ) != std::string::npos
	){
	  c->SetLeftMargin(0.25);
	}

	obj->SetOption("coltextP0");
	obj->SetMarkerSize(6);
	obj->GetYaxis()->SetLabelSize(0.18);
	obj->GetXaxis()->SetLabelSize(0.18);

	if(o.name.find("Missing") != std::string::npos || o.name.find("missing uTCA" ) != std::string::npos){
		double levels[5]={-1.79e308, 0.5, 20, 863, 1.79e308};
		int mycolors[4]={kGreen,kYellow,kRed, 18};
		obj->SetContour(5,levels);
		gStyle->SetPalette(4,mycolors);
		gStyle->SetOptTitle(0);
	}else{
		double levels[4]={-1.79e308, 0.5, 20, 1.79e308};
		int mycolors[3]={kGreen,kYellow,kRed};
		obj->SetContour(4,levels);
		gStyle->SetPalette(3,mycolors);
		gStyle->SetOptTitle(0);
	}
    }
